...
- [ ] Task 2: 新增缓存层并在关键路径接入
  - [ ] 定义缓存接口与内存 TTL 实现（不引入外部依赖）
  - [ ] 在至少一个高频路径接入缓存（例如技能发现/组合计划），并提供开关
  - [ ] 添加缓存命中/过期的单元测试

- [ ] Task 3: 引入进程内事件总线并接入统计/审计点
  - [ ] 实现事件类型基类与 EventBus（异步队列 + 订阅机制）
  - [ ] 在至少一个关键点发布事件（例如技能执行/工作流节点执行）
  - [ ] 添加事件发布与异常隔离的单元测试
...
- [ ] Task 5: 约束健康探针为应用自检（不引入外部探测）
  - [ ] 确认 /health 与 /ready 不包含外部组件连通性探测逻辑
  - [ ] 添加单测防回归（例如 mock 网络函数并断言不被调用）
